...
export const allKeys: cg.Key[] = Array.prototype.concat(
  ...cg.files.map(c => cg.ranks.map(r => c + r)),
);

export const pos2key = (pos: cg.Pos) => allKeys[8 * pos[0] + pos[1] - 9];

export const key2pos = (k: cg.Key) =>
  [k.charCodeAt(0) - 96, k.charCodeAt(1) - 48] as cg.Pos;
...
const posToTranslateBase: (
  ...
  xFactor: number,
  yFactor: number,
) => cg.NumberPair = (pos, asWhite, xFactor, yFactor) => [
...
export const isRightButton = (e: MouseEvent) =>
