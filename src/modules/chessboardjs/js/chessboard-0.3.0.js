...
*/
function customSquareRender(square) {
  const rendered = cfg.renderSquare(<div dangerouslySetInnerHTML={{
    __html: square.innerHTML
  }}></div>, square.getAttribute('data-square'));
...
function drawPositionInstant() {
  ...
  // boardEl.find('.' + CSS.piece).remove(); // old way,
  ...
  boardEl.find('.' + CSS.square).each((index, elem) => {
    elem.innerHTML = customSquareRender(elem);
  });
}
...
function mousedownSquare(e) {
  cfg.onMouseDown && cfg.onMouseDown(e);

  // do nothing if we're not draggable
  if (cfg.draggable !== true || e.button === RIGHT_MOUSE) return;
...
function addEvents() {
  ...
  boardEl.on('contextmenu', '.' + CSS.square, stopEvent);
