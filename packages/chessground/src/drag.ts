...
function processDrag(s: State): void {
  requestAnimationFrame(() => {
    ...
    if (!origPiece || !util.samePiece(origPiece, cur.piece)) cancel(s);
      ...
      if (
        !cur.started &&
        util.distanceSq(cur.epos, cur.rel) >= Math.pow(s.draggable.distance, 2)
      )
        cur.started = true;
      if (cur.started) {
...
export function end(s: State, e: cg.MouchEvent): void {
  ...
  if (dest && cur.started && cur.orig !== dest) {
    ...
    const removedPiece = s.pieces[cur.orig];
    delete s.pieces[cur.orig];
    board.callUserFunction(s.events.change);
    board.callUserFunction(s.events.removePiece, removedPiece, cur.orig);
  }
  if (
    cur &&
